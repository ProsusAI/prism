...
```markdown
# Codebase Directives
*Generated by `/mine-history` (Claude Code) or `@mine-history` (Cursor) on {date}. Analysis window: {window} ({N} commits, {earliest_date} – {latest_date}). Review quarterly.*
...
*Things Claude or Cursor cannot infer from current code alone. Generated by `/mine-history` (Claude Code) or `@mine-history` (Cursor) on {date}. Analysis window: {window} ({earliest_date} – {latest_date}).*
