...
- **Global scope**: `/Users/gaurav/codes/prism/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/gaurav/codes/prism/.claude/notes/`)
...
2. Glob `/Users/gaurav/codes/prism/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/gaurav/codes/prism/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/gaurav/codes/prism/.claude/notes/` — works in any directory
